  ...
  return (
    <aside className="w-64 h-full hidden md:flex flex-col bg-background/50 border-r border-border skeuo-card rounded-none border-t-0 border-l-0 border-b-0">
      ...
      <div className="p-6 flex items-center gap-3">
        <div className="bg-accent/15 p-2.5 rounded-xl text-accent skeuo-inset">
          <Activity size={22} />
        </div>
        <div>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive 
                  ? "skeuo-button text-accent font-semibold border-accent/40 shadow-sm" 
                  : "text-foreground/70 hover:bg-foreground/5 hover:text-foreground"
              )}
            >
              <Icon size={18} className={cn(isActive ? "text-accent" : "text-foreground/50")} />
              {item.name}
            </Link>
          );
        })}
      ...
      <div className="p-6 mt-auto">
        <div className="flex items-center gap-3 p-4 rounded-xl skeuo-inset">
          {/* Physical LED bulb */}
          <div
            className="h-3.5 w-3.5 rounded-full bg-success skeuo-led flex-shrink-0"
